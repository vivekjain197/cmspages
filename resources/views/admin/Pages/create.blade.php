			    ...
			    <div class="col-md-12">
                    ...
                    <div class="panel panel-inverse">
                        ...
                        <div class="panel-body">
                            {{ Form::model($Pages, array('url'=>url('admin/pages/store'), 'method' => 'POST','class'=>'register-form','files'=>true,'id'=>'post_member')) }}
							
							{{-- @foreach($dropdownpages as $dropdownpage)
								<ul>
									<li><a href="">{{$dropdownpage->title}}</a></li>
									@if(count($dropdownpage->subpages))
										@include('admin.Pages.sub_pages',['subages' => $dropdownpage->subpages])
									@endif
								</ul>
					   		@endforeach --}}
							...
							<div class="row">
								...
								<div class="form-group col-md-12">
									...
									<select class="form-control" name="parent_id" id="parent_id">
										...
										@foreach ($dropdownpages as $dropdownpage)
											<option value="{{ $dropdownpage->id }}">{{ $dropdownpage->title }}</option>
											@if(count($dropdownpage->subpages))
												@include('admin.Pages.sub_pages',['subages' => $dropdownpage->subpages,'parent' => $dropdownpage->title,'cmspage'=>''])
											@endif
										@endforeach
									</select>
